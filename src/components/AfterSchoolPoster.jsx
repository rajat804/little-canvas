...
const AfterSchoolPoster = () => {
  ...
  const activities = [
    {
      icon: <GiBlackBelt className="text-2xl" />,
      name: "Karate",
    ...
    <section className="w-full bg-gradient-to-br from-slate-50 via-white to-purple-50 py-20 px-4 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
          ...
          >
            Where learning meets excitement! Discover, create, and grow with our
        ...
        >
          ...
          <div className="flex justify-center px-4 sm:px-6">
            <div className="group relative w-full max-w-4xl rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border-2 sm:border-4 border-white/80 transition-all duration-500 hover:scale-[1.02] hover:shadow-3xl">
              {/* Image Wrapper - Responsive Height */}
              <div className="relative w-full aspect-[4/3] sm:aspect-[16/9] md:aspect-[2/1] lg:aspect-[16/9]">
                <img
              ...
              {/* Floating Activity Tags - Mobile Optimized */}
              <div className="absolute top-2 left-2 sm:top-4 sm:left-4 flex flex-wrap gap-1 sm:gap-2 max-w-[60%] sm:max-w-[70%]">
                {activities.slice(0, 4).map((activity, index) => (
                  <motion.span
                    ...
                    className={`px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold text-white shadow-lg bg-gradient-to-r ${activity.color}`}
                  >
                    {activity.name}
                  </motion.span>
                ))}
              ...
              {/* Main Floating Badge - Mobile Optimized */}
              <motion.div
                initial={{ opacity: 0, scale: 0 }}
                animate={inView ? { opacity: 1, scale: 1 } : {}}
                transition={{ delay: 1.2, type: "spring" }}
                className="absolute top-2 right-2 sm:top-4 sm:right-4 bg-white/95 backdrop-blur-sm px-3 py-1.5 sm:px-5 sm:py-2.5 rounded-xl sm:rounded-2xl shadow-2xl flex items-center gap-1 sm:gap-3 border-2 border-orange-400"
              >
                <span className="text-base sm:text-2xl animate-bounce">🚀</span>
                <div className="text-left">
                  <p className="text-[8px] sm:text-xs text-[#64748B]">
                    Limited Time
                  </p>
                  <p className="text-[10px] sm:text-sm font-bold text-[#1E293B] whitespace-nowrap">
                    Early Bird Discount
                  </p>
                </div>
              </motion.div>
              ...
              <motion.div
                ...
                className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-3 sm:p-6"
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-2.5 bg-gradient-to-r from-orange-500 to-pink-500 text-white font-bold rounded-full shadow-xl text-xs sm:text-sm"
                  >
                    Join Now →
                  </motion.button>
                </div>
